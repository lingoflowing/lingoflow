import { state, getCurrentCard } from './state.js?v=phase128-unified-state-20260629';
...
function placeholderImage(card){
  ...
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 900">
    ...
    <circle
      ...
      opacity="0.72"/>
    <path
      d="M150 705 C290 560 405 610 520 500 C640 385 755 505 870 390 C955 310 1030 342 1095 298 L1095 780 L150 780 Z"
      fill="#e6d8c4"/>
    <text
      x="600"
      y="405"
    ...
    <text
      ...
      font-size="24"
      fill="#9b866d"
      font-family="serif">${no} / image coming soon</text>
  </svg>`;
...
function renderKey(card, src){
  const id =
    safeText(card?.id) ||
    safeText(card?.cardId) ||
    safeText(card?.cardNo) ||
...
function renderTextImmediately(card){
  ...
  // previous .is-changing / transition state.
  // This is intentionally limited to first render and same-card re-render.
  if(textArea){
...
function removeExistingOverlay(){
  ...
  document
    .querySelectorAll('.photo-fade-overlay')
...
function makeOverlayFromCurrentImage(){
  ...
  overlay.style.transition =
    `opacity ${IMAGE_CROSSFADE_MS}ms cubic-bezier(.45,0,.2,1)`;
...
function crossfadeToImage(src, card){
  ...
  // The main image remains fully visible.
  // We change only its src.
  // The old image is preserved in the overlay
  // and fades out above it.
  photo.classList.remove('is-changing');
  photo.style.opacity = '1';
  photo.style.visibility = 'visible';
  photo.src = src;
...
function setCardWithFade(card){
  ...
  // This prevents the blank screen caused by waiting for a fade
  // before any image/text has ever been displayed.
  if(!hasRenderedOnce){
    ...
    // Keep .is-initializing until the image load event.
    // Removing it here exposes the alt text before the image is ready.
    return;
  }

  // Re-rendering the same first/current card must not fade
  // the word/sentence.
  // This happens when playback starts:
  // app.js calls renderCurrentCard() again even though
  // the first card is already visible from page load.
  ...
  if(textArea){
    textArea.classList.add('is-changing');
  }
